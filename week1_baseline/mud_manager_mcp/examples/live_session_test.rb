...
require_relative "../lib/mud_manager_mcp"
...
class LiveSessionTest < Minitest::Test
  HOST = ENV.fetch("MUD_HOST", MudManagerMcp::Session::DEFAULT_HOST)
  PORT = Integer(ENV.fetch("MUD_PORT", MudManagerMcp::Session::DEFAULT_PORT))
  ...
  def test_login_and_read_only_player_commands
    ...
    with_logged_in_session do |session|
      command_examples = [
        MudManagerMcp::Primitives.look,
        MudManagerMcp::Primitives.info_self("exits"),
        MudManagerMcp::Primitives.info_self("inventory"),
        MudManagerMcp::Primitives.info_self("equipment"),
        MudManagerMcp::Primitives.info_self("score"),
        MudManagerMcp::Primitives.info_world("who"),
        MudManagerMcp::Primitives.info_world("help", filter: "look")
      ]
  ...
  def test_reports_wrong_password_as_login_error
    ...
    with_open_session do |session|
      error = assert_raises(MudManagerMcp::Session::LoginError) do
  ...
  def with_open_session
    ...
    session.open
    yield session
  rescue MudManagerMcp::Session::ConnectionError => e
  ...
  def with_logged_in_session
    with_open_session do |session|
      ...
      rescue MudManagerMcp::Session::LoginError => e
