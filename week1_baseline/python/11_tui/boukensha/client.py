...
class Client:
    ...
    def call(
        ...
        # http.client rejects a None header value outright (TypeError deep in
        # putheader), unlike Ruby's Net::HTTP which tolerates it and lets the
        # request go out — so a missing api_key surfaces as a clean 401 from
        # the server (caught below as ApiError) instead of a crash here.
        headers = {k: v for k, v in self._builder.headers().items() if v is not None}
