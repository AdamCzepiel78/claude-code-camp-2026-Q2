...
Every backend's ``parse_response`` returns::
    ...
    {"stop_reason": "tool_use" | "end_turn", "content": [<block>, <block>, ...]}

where each block is one of::

    {"type": "reasoning", "text": "<may be empty>", "signature": "...", "redacted": True|False}
    {"type": "text", "text": "..."}
...
This is plumbing, not a feature switch: no backend in this codebase requests
visible reasoning by default (Gemini explicitly disables it via
``thinkingConfig``, Ollama/OllamaCloud send ``think: False``, OpenAI sends
``reasoning={"effort": "none"}``, Anthropic never requests it at all) — the
contract exists so a reasoning block is handled correctly if one ever
appears, not because this step turns thinking on.
"""
