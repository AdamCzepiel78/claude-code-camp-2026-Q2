...
class Gemini(Base):
    ...
    # Normalizes a Gemini generateContent response into the common shape (see
    # Backends.Base for the full content-block contract).
    #
    ...
    # of parse_response). Text-only turns are stored as a bare string, so wrap
    # it back into a single text block before mapping.
    @staticmethod
