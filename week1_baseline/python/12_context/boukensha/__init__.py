"""Boukensha — Step 12: Context Management.
...
- **The ``Tasks::Base``/``Tasks::Player`` indirection is gone.** ``Config`` now
  exposes ``provider_type``/``model``/``system_prompt`` and the agent's limit
  settings (``agent_max_iterations``, ``agent_max_output_tokens``,
  ``agent_max_turn_tokens``, ``agent_compaction_threshold``) directly — no more
  per-task settings dict passed around, no more ``Context.task``.
- **Context-window tracking and auto-compaction** — ``run``/``repl`` gain a
  ``context_window: int | None = None`` keyword; when not given, it's read
  from the constructed backend's own ``context_window`` (see below), not a
  separate lookup table.
- **The backend is now constructed *before* ``Context``**, a deliberate
  reordering from step 11 (where ``Context`` came first). Nothing between the
  two needs the backend built later — checked directly against the previous
  construction order — and building it first means ``context_window`` can
  come from ``backend.context_window`` (the backend's own per-model table,
  always correct) instead of a name-only lookup that can't tell two backends'
  same-named models apart (this port keeps ``mammouth``, whose model catalogue
  overlaps Anthropic's own model names with *different* context windows —
  see :mod:`boukensha.models` for the full reasoning this reorder replaces).
- **``mammouth`` stays** — a deliberate deviation from Ruby's ``12_context``,
  which drops the backend entirely. Kept here on request; nothing else about
  this step depends on that choice.
...
``repl()`` also takes ``tui: bool = True``: by default it wraps the plain REPL
in a full-screen terminal UI (:mod:`boukensha.tui`, built on Textual). Pass
``tui=False`` for the plain terminal loop instead.
"""
...
def run(
    *,
    ...
    Options mirror the Ruby source; unset values fall back to ``Config``. See
    the module docstring for ``working_dir``/``allowed_commands``/
    ``shell_timeout``/``mud``.
    """
    ...
    resolved_context_window = context_window or be.context_window
