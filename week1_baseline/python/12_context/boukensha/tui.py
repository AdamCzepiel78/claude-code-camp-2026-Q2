...
class BoukenshaApp(App):
    ...
    def _handle_event(self, run_id: int, event: dict[str, Any]) -> None:
        ...
        if phase == "iteration":
            ...
            self._append_log(f"[context compacted — {dropped} messages dropped to free space]")
    ...
    def _render_progress(self) -> None:
        ...
        if self._live["active"]:
            ...
            maxw = self._fmt_tokens(context.context_window)
            widget.update(
                f"[{color}]  [ready]   ctx {used} / {maxw} ({pct}%)   {self._turn_count} turns[/{color}]"
            )
    ...
    def _render_status(self) -> None:
        ...
        bar = (
            f" boukensha v{ver} · {model}  ·  ctx {used}/{maxw} ({pct}%){ctx_indicator}"
            f"·  {tools} tools  ·  {clock} "
        )
        widget.update(bar)
