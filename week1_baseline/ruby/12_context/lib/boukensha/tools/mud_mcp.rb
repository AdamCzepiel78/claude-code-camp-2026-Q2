  ...
  module Tools
    ...
    module MudMcp
      ...
      def default_command
        ...
        # Prefer running straight from a checkout — no `gem install` needed. When
        # this is itself running as an installed gem, that relative path does not
        # exist, so fall back to the mud_manager_mcp gem's own executable (the
        # dependency guarantees it is present).
        return ["ruby", DEFAULT_SERVER] if File.exist?(DEFAULT_SERVER)

        require "rubygems"
        ["ruby", Gem.bin_path("mud_manager_mcp", "mud_manager_mcp")]
      end
